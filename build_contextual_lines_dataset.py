...
def example_is_valid(context, imports, line, min_length=5, max_length=80):
...
def build_examples_from_file(f, context_size=10):
    ...
    for line_raw in f.split('\n'):
        ...
        c, i = ids[context_size:], list(imports)
        if example_is_valid(c, i, line):
            examples.append({ 'c': c, 'i': i, 'l': line, 'p': lines[-context_size:]})
...
def build_dataset(files, context_size=10, examples_from_file=20, progress=None):
    examples = []

    for f in files:
        f_examples = build_examples_from_file(f, context_size)
        examples.extend(
                random.sample(f_examples,
                              min(len(f_examples), examples_from_file)))
    ...
    with open(opt.files_dataset) as f:
        ...
        for split, files in files_dataset.items():
            dataset[split] = build_dataset(
                    random.sample(files, int(n / files_count * len(files))),
                    opt.context_size,
                    p)
